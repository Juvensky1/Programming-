...
public:
   ...
   ostream& preorderTraverseAndWrite(const Position<T>& p, ostream& out ) const;
private:
   Node<T>* superRootPtr;
...
template <class T>
ostream& BinarySearchTree<T>::preorderTraverseAndWrite(const Position<T>& p, ostream& out ) const {
    if (p.isInternal()) {
         p.nodePtr->item.write(out);
        preorderTraverseAndWrite(p.nodePtr->leftPtr, out);
        preorderTraverseAndWrite(p.nodePtr->rightPtr, out);
    }
    return out;
}
